...
disable_bbr_ecn
echo 2 > $MOD_PARAM_DIR/cwnd_min_cap
...
if [[ $tests == *"ecn_bulk"* ]]; then
    ...
    enable_bbr_ecn
    for rep in `seq 1 10`; do
	for cc_name in dctcp bbr2_min_2 bbr2_min_4 bbr; do
		...
		cc=${cc_name}:${num_flows}  # all flows bbr2
		if [[ $cc_name == "bbr2_min_2" ]]; then
		    echo 2 > $MOD_PARAM_DIR/cwnd_min_cap
		    cc="bbr2:${num_flows}"
		fi
		if [[ $cc_name == "bbr2_min_4" ]]; then
		    echo 4 > $MOD_PARAM_DIR/cwnd_min_cap
		    cc="bbr2:${num_flows}"
		fi
